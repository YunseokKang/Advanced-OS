...
void timer_sleep(int64_t ticks)
{
    ...
    if (0 < ticks)
    {
      ...
      current_thread->wakeUpTick = start + ticks;
      list_insert_ordered(&sleep_list, &current_thread->elem, wake_up_tick_less, NULL);
...
static void timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  thread_tick ();
    ...
    while (!list_empty(&sleep_list) && list_entry(list_front(&sleep_list), struct thread, elem)->wakeUpTick <= ticks) //slee_list is maintained in a sorted state. 
    {

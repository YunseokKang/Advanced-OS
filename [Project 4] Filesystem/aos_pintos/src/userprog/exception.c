...
//     kill (f);
   /* Handle the case where the page fault is due to stack expansion. */
   if (is_user_vaddr(fault_addr)
      && (uint8_t *)(fault_addr) >= (uint8_t *)(f->esp) - 32
      && fault_addr >= STACK_LIMIT)
   page_alloc(fault_addr);

   /* Resolve the page fault using virtual memory management; terminate the process if unresolved. */
   if (!page_resolve_fault(fault_addr))
   kill(f);

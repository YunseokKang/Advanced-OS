...
void *pagedir_get_page (uint32_t *pd, const void *uaddr)
{
  ...
  if (pte != NULL && (*pte & PTE_P) != 0)
    return (void *)((char *) pte_get_page(*pte) + pg_ofs(uaddr));
  else
    return NULL;
}
...
// added
/* Update the writable bit (PTE_W) in the Page Table Entry (PTE) for a given virtual page (VPAGE)
   in the Page Directory (PD) based on the 'writable' flag. */
void pagedir_set_writable (uint32_t *pd, const void *vpage, bool writable)
{
  // Look up the page table entry for the given virtual page.
  uint32_t *pte = lookup_page (pd, vpage, false);
  if (pte != NULL) // Check if the page table entry was successfully retrieved.
    {
      if (writable)
        *pte |= PTE_W; // Set the writable bit if 'writable' is true.
      else
        {
          *pte &= ~(uint32_t) PTE_W; // Clear the writable bit if 'writable' is false.
          invalidate_pagedir (pd); // Invalidate the page directory to update the hardware cache.
        }
    }
}
